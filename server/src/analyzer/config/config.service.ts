import { Diagnostic, DiagnosticSeverity, TextDocumentPositionParams, CompletionItem } from 'vscode-languageserver';
...
export class ConfigService {
  ...
  public validate(textDocument: TextDocument): Diagnostic[] | void {
    ...
    if (!(Array.isArray(json))) {
      diagnostics.push(this.createErrorMessage(textDocument, 'The wrapping structure needs to be an array', 0, this.text.length));
    }

    // We need at least one other service to validate against
    if (json.length === 0) {
      diagnostics.push(this.createErrorMessage(textDocument, 'There needs to be defined a single service at least', 0, this.text.length));
    }
    ...
    for (const service of json) {
      ...
      if (typeof service !== 'object' || Array.isArray(service)) {
        if (typeof service === 'string' || typeof service === 'number' || typeof service === 'boolean') {
          if (typeof service === 'string' && service === '') {
            currentBlockStartIndex = this.text.indexOf('""', currentBlockEndIndex);
            currentBlockEndIndex = currentBlockStartIndex + 2;
          } else if (typeof service === 'string') {
            const thing = `"${String(service)}"`;
            currentBlockStartIndex = this.text.indexOf(thing, currentBlockEndIndex);
          ...
          currentBlockStartIndex = this.text.indexOf('[', currentBlockEndIndex);
          currentBlockEndIndex = this.findIndexOfClosingArray(this.text, currentBlockStartIndex);
        }
      ...
      currentBlockStartIndex = this.text.indexOf('{', currentBlockEndIndex);
      currentBlockEndIndex = this.findIndexOfClosingBlock(this.text, currentBlockStartIndex);
      // Check the name
      if (!service.name) {
        ...
        currentBlockStartIndex = this.text.indexOf('"name":', currentBlockStartIndex);
        currentBlockEndIndex = this.text.indexOf(',', currentBlockStartIndex);
      ...
      if (!service.baseUri) {
        ...
        if (!regEex.test(service.baseUri)) {
          const uriIndex = this.text.indexOf('"baseUri":', currentBlockStartIndex);
      ...
      if (!service.language) {
        ...
        if (service.language !== 'Typescript' && service.language !== 'Java') {
          const languageIndex = this.text.indexOf('"language":', currentBlockStartIndex);
      ...
      if (!service.lib) {
        ...
        const libIndex = this.text.indexOf('"lib":', currentBlockStartIndex);
        if (service.language) {
      ...
      if (service.endpoints && !Array.isArray(service.endpoints)) {
        const endpointsIndexStart = this.text.indexOf(`"endpoints":`, currentBlockStartIndex);
      ...
      if (service.endpoints && service.endpoints.length === 0) {
        const endpointsIndexStart = this.text.indexOf(`"endpoints":`, currentBlockStartIndex);
        const endpointsIndexEnd = this.text.indexOf(']', endpointsIndexStart);
      ...
      if (service.endpoints && Array.isArray(service.endpoints)) {
        ...
        let endpointStartIndex = this.text.indexOf('[', currentBlockStartIndex) + 1;
        ...
        for (const endpoint of service.endpoints) {
          ...
          if (typeof endpoint !== 'object' || Array.isArray(endpoint)) {
            if (typeof endpoint === 'string' || typeof endpoint === 'number' || typeof endpoint === 'boolean') {
              if (typeof endpoint === 'string' && endpoint === '') {
                endpointStartIndex = this.text.indexOf('""', endpointEndIndex);
                endpointEndIndex = endpointStartIndex + 2;
              } else if (typeof endpoint === 'string') {
                const thing = `"${String(endpoint)}"`;
                endpointStartIndex = this.text.indexOf(thing, endpointEndIndex);
              ...
              endpointStartIndex = this.text.indexOf('[', endpointEndIndex);
              endpointEndIndex = this.findIndexOfClosingArray(this.text, endpointStartIndex);
            }
            diagnostics.push(this.createErrorMessage(textDocument, 'Endpoint needs to be an object', endpointStartIndex, endpointEndIndex));
            continue;
          }
          endpointStartIndex = this.text.indexOf('{', endpointEndIndex);
          endpointEndIndex = this.findIndexOfClosingBlock(this.text, endpointStartIndex);
          // Check the method
          if (!endpoint.method) {
            ...
            if (endpoint.method !== 'GET' && endpoint.method !== 'POST' && endpoint.method !== 'PUT' && endpoint.method !== 'DELETE') {
                endpointStartIndex = this.text.indexOf('"method":', endpointStartIndex);
          ...
          if (!endpoint.path) {
            ...
            if (!regEex.test(endpoint.path)) {
              const pathIndex = this.text.indexOf('"path":', endpointStartIndex);
          ...
          if (!endpoint.response) {
            ...
            const responseStartIndex = this.text.indexOf('"response":', endpointStartIndex);
            if (typeof endpoint.response === 'string') {
              ...
              for (const attr of attrs) {
                ...
                const attrIndex = this.text.indexOf(attr, responseStartIndex);
          ...
          if (!endpoint.request) {
            ...
            const requestStartIndex = this.text.indexOf('"request":', endpointStartIndex);
            if (typeof endpoint.request !== 'object' || Array.isArray(endpoint.request)) {
              ...
              for (const attr of attrs) {
                ...
                const attrIndex = this.text.indexOf(attr, requestStartIndex);
                if (typeof val === 'object' && !Array.isArray(val)) {
